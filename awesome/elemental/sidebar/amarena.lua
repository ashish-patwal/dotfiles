...
local anya = os.getenv("HOME") .. "/.config/awesome/covers/" .. math.random(0, 8) .. ".png"

-- github streak
local github_streak = require("noodle.github.github-contributions-widget")

-- word clock
local word_clock = require("noodle.word-clock")

-- Moe
local moe = require("noodle.moe")
local moe_widget_children = moe:get_all_children()
local moe_title = moe_widget_children[5]
local moe_artist = moe_widget_children[7]
moe_title.forced_height = dpi(22)
moe_artist.forced_height = dpi(16)
...
music_sidebar.fg = beautiful.sidebar_fg or beautiful.wibar_fg or "#FFFFFF"
music_sidebar.opacity = beautiful.sidebar_opacity or 1
music_sidebar.height = screen.primary.geometry.height / 1.5
music_sidebar.width = beautiful.sidebar_width or dpi(300)
music_sidebar.y = beautiful.sidebar_y or 0
if beautiful.sidebar_position == "right" then
  awful.placement.left(music_sidebar)
else
...
-- Hide music sidebar when mouse leaves
if user.sidebar.hide_on_mouse_leave then
...
-- Activate sidebar by moving the mouse at the edge of the screen
if user.sidebar.show_on_mouse_screen_edge then
  local music_sidebar_activator = wibox({ y = music_sidebar.y, width = 1, visible = true, ontop = false, opacity = 0,
    below = true, screen = screen.primary })
  ...
  if beautiful.sidebar_position == "right" then
    awful.placement.left(music_sidebar_activator)
  else
    awful.placement.right(music_sidebar_activator)
  end

  -- music_sidebar_activator:buttons(
  --     gears.table.join(
  --         awful.button({ }, 4, function ()
  --             awful.tag.viewprev()
  --         end),
