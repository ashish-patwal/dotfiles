...
awful.placement.top(w, { margins = { top = 10 } })

local function translate(to_translate, lang)

  local translation_icon = wibox.widget.imagebox()
  local cmd = "trans " .. lang .. ' "' .. to_translate .. '" --no-ansi --show-original n'
  ...
  awful.spawn.easy_async_with_shell(cmd, function(stdout, stderr)
      ...
      {
        {
          {
            {
              image  = os.getenv("HOME") .. "/.config/awesome/covers/translation.svg",
              widget = translation_icon,
              resize = false
            },
            valign = 'top',
    ...
    translation_icon:add_button(awful.button({}, 1, function()
      -- naughty.notify { text = cmd }
      awful.spawn.with_shell("trans " .. lang .. ' "' .. to_translate .. '" -p')
    end))
    ...
    w:buttons(
      awful.util.table.join(
        awful.button({}, 1, function()
          awful.spawn.with_shell("echo '" .. stdout .. "' | xclip -selection clipboard")
          -- w.visible = false
        end),
...
function helpers.prompt(action, textbox, prompt, callback)
  ...
  elseif action == "translate" then
    awful.prompt.run {
      ...
      exe_callback  = function(input)
        if not input or #input == 0 then return end
        local to_translate, lang = extract(input)
        ...
        -- awful.spawn.with_shell(user.web_search_cmd .. input .. "'")
        -- awful.spawn.with_shell("noglob "..user.web_search_cmd.."'"..input.."'")
        -- naughty.notify { title = lang, text = to_translate }
      end
    }
  end
end
